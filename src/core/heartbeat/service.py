...
class HeartbeatService:
    ...
    async def start(self, agent_ids: Optional[List[str]] = None) -> None:
        ...
        if agent_ids:
            ...
            default_agents_str = os.getenv(
                "SOVEREIGN_DEFAULT_AGENTS", "echo,ryuzu,beatrice"
            )
            default_agents = [
                a.strip() for a in default_agents_str.split(",") if a.strip()
            ]
        ...
        logger.info(
            f"=== HEARTBEAT SERVICE ONLINE: {len(self._registered_agents)} agents ==="
        )
    ...
    async def stop(self) -> None:
        ...
        for agent_id in self._registered_agents:
            try:
                ...
                history = await self._cache.get_messages(
                    f"session_{agent_id}", limit=20
                )
    ...
    async def _restore_agent_state(self, agent_id: str):
        ...
        try:
            ...
            if snapshot:
                ...
                logger.debug(
                    f"No stasis snapshot found for {agent_id}. Starting fresh."
                )
        except Exception as e:
    ...
    async def trigger_once(
        self, agent_id: str, user_message: Optional[str] = None
    ) -> dict:
        """
