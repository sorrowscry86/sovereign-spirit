...
class DatabaseClient:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            if ("getaddrinfo failed" in error_msg or "could not translate host name" in error_msg) and "localhost" not in str(self._engine.url):
                logger.warning("Database connection failed for Docker host, trying localhost fallback...")
                try:
                    # Construct localhost URL - MUST use render_as_string(hide_password=False)
                    # otherwise SQLAlchemy masks the password as '***'
                    original_url = self._engine.url.render_as_string(hide_password=False)
                    ...
                    logger.error(f"Postgres localhost fallback also failed: {fallback_e}")
